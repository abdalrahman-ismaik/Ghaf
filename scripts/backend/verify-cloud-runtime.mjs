...
const evidenceScope =
  'Feature019 and four shared baseline migrations only, in memory with a minimal Supabase Auth SQL facade';
const scopedMigrations = [
  '20260913000100_pilot_access.sql',
...
const scopedSuites = [
  'account_profiles.test.sql',
  'account_provider_status.test.sql',
  'account_workspaces.test.sql',
  'normalized_family_extensions.test.sql',
  'normalized_family_runtime.test.sql',
  'pilot_access.test.sql',
];
...
function snapshotValidator() {
  ...
  const model = compile('src/models/normalizedCloudFamily.ts');
  return compile('src/features/cloudFamily/validation.ts', (name) =>
    name === '../../models/normalizedCloudFamily' ? model : require(name),
  );
}
...
function readSqlFiles(directory, pattern, requiredNames) {
  ...
  if (requiredNames.some((name) => !names.includes(name))) {
    throw new VerificationError('missing_required_sql_file');
  }
  const selected = requiredNames.map((name) => {
    const source = readFileSync(resolve(repositoryRoot, directory, name), 'utf8');
  ...
  return {
    selected,
    names,
    excluded: names.filter((name) => !requiredNames.includes(name)),
  };
}
...
export async function verifyCloudRuntime() {
  const receipt = {
    ...
    native: 'NOT RUN',
    combinedFamilyRuntime: 'NOT RUN',
    cron: 'NOT RUN',
    requiredMigrations: scopedMigrations,
    requiredSuites: scopedSuites,
    excludedMigrations: [],
    excludedSuites: [],
    status: 'FAILED',
    stage: 'preflight',
  ...
  console.log(
    '[sql-only] Feature020, Cron, GoTrue, JWT signature verification, hosted access and native tests are NOT RUN.',
  );
  try {
    const validators = snapshotValidator();
    const migrationFiles = readSqlFiles(
      'supabase/migrations',
      /^\d{14}_[a-z0-9_]+\.sql$/u,
      scopedMigrations,
    );
    const suiteFiles = readSqlFiles(
      'supabase/tests/database',
      /^[a-z0-9_-]+\.test\.sql$/u,
      scopedSuites,
    );
    const migrations = migrationFiles.selected;
    const suites = suiteFiles.selected;
    receipt.excludedMigrations = migrationFiles.excluded;
    receipt.excludedSuites = suiteFiles.excluded;
    console.log(
      `[sql-only] Selected ${migrations.length} migrations and ${suites.length} suites; excluded ${migrationFiles.excluded.length} migrations and ${suiteFiles.excluded.length} suites.`,
    );
    for (const suite of suites) requireRollbackSuite(suite.source);
    if (
      new Set(migrationFiles.names.map((name) => name.slice(0, 14))).size !==
