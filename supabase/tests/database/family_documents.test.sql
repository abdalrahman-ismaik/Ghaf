...
select throws_ok($$select pg_temp.doc_command(jsonb_set((select value from document_state where key='create-command'),'{command,input,title}','"Changed"'),'020d4000-0000-4000-8000-000000000001')$$,'PT409','request_conflict','A request UUID cannot bind new wording');
...
select throws_ok($$select pg_temp.doc_command(jsonb_set((select value from document_state where key='create-command'),'{command,input,dueDate}','"2026-02-30"'))$$,'PT409','request_conflict','Existing plan ID cannot be overwritten by another create');
...
select throws_ok($$select pg_temp.doc_command('{"type":"template.save","id":null,"expectedRevision":0,"input":{"categoryId":"home_responsibility","title":{"ar":"ترتيب الكتب","en":"Arrange books"},"positiveAction":{"ar":"ضع الكتب في مكانها","en":"Put agreed books away"},"recurrence":"once"}}')$$,'PT409','request_conflict','Duplicate saved wording is rejected under the family lock');
