...
| Child checklist progress, `r002a/child/ChildTaskChecklist.tsx`                                 | `width: ${progress}%` recomputed per toggle.                                                                                    | The core child loop jumped between discrete widths, so the advance was never visible as an advance.               | Transform-driven fill settled with the shared progress preset, anchored at the start edge. | The step the child just finished is legible as travel.          | Low–medium; RTL anchor, verified in tests.     |
| Child checklist row state, same file                                                           | Row background, border and the check icon all switched on the same render; the icon was conditionally mounted.                  | The completed state appeared with no relationship to the tap that caused it, and a remount could pop the icon in. | `interpolateColor` tint plus a mounted mark that settles with the same value.              | One readable completion, no entrance replay on remount.         | Low; colors are light-on-light throughout.     |
