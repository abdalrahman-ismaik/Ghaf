...
| `npm test`                                                                                          | **PASSED** with one identified intermittent failure — 3,437 passed, 7 skipped, 238 files. Two of seven parallel runs failed in `tests/demo/demo-entry-routes.test.tsx` only: one run hit the 5,000 ms per-test timeout and the next case then read a null render. That suite calls `vi.resetModules()` and re-imports six route modules per test, and it mocks `@/components/primitives`, `@/components/r002a`, `@/components/r003`, `@/components/access` and `@/utils/useReducedMotionPreference`, which are the only paths from those routes to anything this pass changed, so its graph does not load the changed modules. It passed 3/3 in isolation, in the serial run, and in three full runs at `35631f7`; treat it as a host-timing sensitivity in that suite, not as a verified pre-existing defect and not as a cleared one. |
