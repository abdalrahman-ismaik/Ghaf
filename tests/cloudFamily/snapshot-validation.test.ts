...
import { CloudFamilyError, type CloudSnapshot } from '../../src/models/normalizedCloudFamily';
...
describe('normalized cloud response boundary', () => {
  ...
  it('reads normalized goals with omitted dates as undated without changing their saved evidence', () => {
    const value = snapshot();
    const goal = value.extras.goals[0]!;
    const savedGoal = Object.fromEntries(
      Object.entries(goal).filter(([key]) => key !== 'targetDate' && key !== 'reviewDate'),
    );
    const parsed = parseCloudSnapshot(
      { ...value, extras: { ...value.extras, goals: [savedGoal] } },
      parent,
    );
    expect(parsed.extras.goals[0]).toEqual({ ...savedGoal, targetDate: null, reviewDate: null });
    expect(savedGoal).not.toHaveProperty('targetDate');
    expect(savedGoal).not.toHaveProperty('reviewDate');
  });

  it('validates explicit academic dates and refuses an invalid review order', () => {
    const value = snapshot();
    const goal = value.extras.goals[0]!;
    goal.targetDate = '2026-09-15';
    goal.reviewDate = '2026-09-16';
    expect(parseCloudSnapshot(value, parent).extras.goals[0]).toMatchObject({
      targetDate: '2026-09-15',
      reviewDate: '2026-09-16',
    });
    goal.reviewDate = goal.targetDate;
    expect(parseCloudSnapshot(value, parent).extras.goals[0]!.reviewDate).toBe('2026-09-15');
    goal.reviewDate = '2026-09-14';
    rejects(value);
    goal.targetDate = null;
    expect(parseCloudSnapshot(value, parent).extras.goals[0]!.reviewDate).toBe('2026-09-14');
    goal.reviewDate = '2026-02-30';
    rejects(value);
  });
