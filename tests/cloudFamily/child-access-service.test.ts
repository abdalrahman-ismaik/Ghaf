...
const modules = vi.hoisted(() => ({
  ...
  projectUrl: 'https://synthetic-ghaf.supabase.co',
}));
...
vi.mock('../../src/features/pilot/config', () => ({
  getPilotConfig: () => ({
    ...
    valid: true,
    mode: 'supabase',
    supabaseUrl: modules.projectUrl,
    supabasePublishableKey: 'sb_publishable_synthetic_child_tests',
  }),
}));

function projectKey(key: string, url = modules.projectUrl) {
  const endpoint = new URL(url).href.replace(/\/+$/, '');
  const encoded = Array.from(endpoint, (character) =>
    character.charCodeAt(0).toString(16).padStart(4, '0'),
  ).join('');
  return ACCOUNT_STORAGE_KEY + '.project.v1.' + encoded + '.' + key;
}
...
function harness(hasSession = true) {
  ...
  if (hasSession) records.set(projectKey(CHILD_STORAGE_KEY), JSON.stringify(session));
  const rawStorage = {
...
beforeEach(() => {
  modules.projectUrl = 'https://synthetic-ghaf.supabase.co';
...
describe('separate Supabase Child access service', () => {
  it('never restores a different project or legacy unscoped Child credential', async () => {
    const h = harness();
    h.records.set(CHILD_STORAGE_KEY, JSON.stringify(session));
    modules.projectUrl = 'https://different-synthetic-ghaf.supabase.co';
  ...
  it('keeps device preference project-scoped without moving or removing the other project session', async () => {
    const h = harness();
    const firstProject = modules.projectUrl;
    await writeCloudDeviceMode('child');
    modules.projectUrl = 'https://different-synthetic-ghaf.supabase.co';
    await expect(readCloudDeviceMode()).resolves.toBe('parent');
    expect(h.records.has(projectKey(CHILD_STORAGE_KEY, firstProject))).toBe(true);
    modules.projectUrl = firstProject;
  ...
  it('restores a verified provider identity from the Child namespace without touching Parent credentials', async () => {
    ...
    expect(
      h.rawStorage.getItem.mock.calls.every(([key]) => key === projectKey(CHILD_STORAGE_KEY)),
    ).toBe(true);
  ...
  it('creates a distinct anonymous identity and sends only the enrollment token to server authority', async () => {
    ...
    expect(h.records.get(ACCOUNT_STORAGE_KEY)).toBe('untouched-parent-session');
    expect(h.records.has(projectKey(CHILD_STORAGE_KEY))).toBe(true);
  });
  ...
  it('invalidates an outstanding read when logout wins the race', async () => {
    ...
    await rejected;
    expect(h.records.has(projectKey(CHILD_STORAGE_KEY))).toBe(false);
    expect(h.records.get(ACCOUNT_STORAGE_KEY)).toBe('untouched-parent-session');
  });
  ...
  it('reports failed credential clearing and keeps subsequent Child access closed', async () => {
    ...
    h.rawStorage.removeItem.mockImplementation(async (key) => {
      if (key === projectKey(CHILD_STORAGE_KEY)) throw new Error('synthetic-storage-locked');
  ...
  it('treats the device-mode marker as navigation preference without creating identity authority', async () => {
    ...
    expect(h.client.auth.signInAnonymously).not.toHaveBeenCalled();
    expect(h.client.rpc).not.toHaveBeenCalled();
    h.records.set(projectKey(`${ACCOUNT_STORAGE_KEY}.device-mode`), 'Salem');
    await expect(readCloudDeviceMode()).resolves.toBe('parent');
    expect(h.records.get(ACCOUNT_STORAGE_KEY)).toBe('untouched-parent-session');
  });
});
