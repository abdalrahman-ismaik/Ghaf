...
describe('R003 complete screen journey', () => {
  ...
  it('uses separate access paths and retires the role toggle', () => {
    ...
    expect(parentSignIn).toContain('beginLocalFamilyProfileRepair');
    ...
    expect(parentSignUp).toContain('beginLocalFamilySetup');
    expect(parentSignUp).not.toContain('<PrototypePill');
    expect(parentSignUp).not.toContain('flow=create-family');
    expect(parentSignUp).toContain('parentOnboarding.completionReceipt');
    expect(parentSignUp).toContain("router.replace('/access/parent/family-basics')");
    expect(parentVerification).not.toContain('useLocalSearchParams');
    expect(parentVerification).toContain('<Redirect href="/access/parent/sign-in" />');
    expect(parentVerification).toContain('parentOnboarding.completionReceipt');
    expect(parentVerification).not.toContain('completeParentOnboarding');
    expect(parentVerification).not.toContain('<OtpInput');
    expect(parentVerification).toContain("'/parent'");
    expect(parentVerification).toContain('<Redirect href="/child" />');
