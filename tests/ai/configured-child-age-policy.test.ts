...
describe('configured Child age authority', () => {
  ...
  it.each(['6_8', '9_11', '12_14'] as const)(
    ...
    (ageBand) => {
      ...
      expect(localFamily.record).toMatchObject({
        schemaVersion: 4,
        children: [{ sex: 'male', ageBand }, { sex: 'female' }],
      });
  ...
  it.each(['missing_sex', 'legacy_gender', 'missing_guardian'] as const)(
    'rejects %s in the current family schema before deriving configured age',
    (condition) => {
      const view = structuredClone(usePrototypeStore.getState().localFamily);
      const record = view.record!;
      const { sex: _sex, ...legacyChild } = record.children[0]!;
      const altered = {
        ...view,
        record: {
          ...record,
          ...(condition === 'missing_guardian'
            ? { familyConnections: { ...record.familyConnections, primaryGuardianName: '' } }
            : {
                children: [
                  condition === 'legacy_gender' ? { ...legacyChild, gender: 'boy' } : legacyChild,
                  ...record.children.slice(1),
                ],
              }),
        },
      };
      expect(
        resolveConfiguredChildAgeBand(altered as unknown as LocalFamilyView, 'child_salem'),
      ).toBeNull();
    },
  );
