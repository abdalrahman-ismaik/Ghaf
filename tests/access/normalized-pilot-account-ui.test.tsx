...
import { NormalizedPilotAccountView } from '../../src/components/pilot/NormalizedPilotAccountView';
import {
  EnabledNormalizedPilotGate,
  NormalizedPilotGate,
} from '../../src/components/pilot/NormalizedPilotGate';
...
function renderView(children?: ReactNode) {
  const shell = NormalizedPilotAccountView({
...
function renderGate() {
  mock.cursor = 0;
  tree = EnabledNormalizedPilotGate({
...
describe('normalized Parent runtime identity boundary', () => {
  it('withholds normalized family and sample content for a restored hosted Child session', () => {
    mock.pilot = {
      ...mock.pilot,
      phase: 'ready',
      sampleOpen: true,
      account: {
        ...
        email: '',
        role: 'child',
        familyId: 'hosted-family',
        childId: 'profile',
      },
    };
    mock.state.pilotSampleActive = true;
    renderGate();
    expect(byId('normalized-incompatible-child-session')).toBeDefined();
    expect(byId('private-demo-tree')).toBeUndefined();
    expect(find((node) => node.type === NormalizedPilotAccountView)).toBeUndefined();
    expect(find((node) => node.props.userId === 'hosted-child')).toBeUndefined();
  });
});
...
describe('Pilot navigator boundary', () => {
  ...
  it('keeps the same outer view across auth phases and retains the workspace identity boundary', () => {
    renderGate();
    const signin = find((node) => node.type === NormalizedPilotAccountView)!;
    mock.pilot = {
      ...mock.pilot,
      phase: 'ready',
      account: { userId: 'adult-a', email: 'a@example.test' },
    ...
    renderGate();
    const ready = find((node) => node.type === NormalizedPilotAccountView)!;
    ...
    expect(find((node) => node.type === NormalizedPilotAccountView)?.key).toBeNull();
  });
  ...
  it('returns the default demo without constructing an account service', () => {
    ...
    expect(NormalizedPilotGate({ children: child })).toBe(child);
  ...
  it('renders configuration failure without a service or any protected child tree', () => {
    ...
    expect(mock.getService).not.toHaveBeenCalled();
    expect(byId('private-demo-tree')).toBeUndefined();
    expect(find((node) => node.type === NormalizedPilotAccountView)?.props.state).toMatchObject({
      phase: 'configuration',
    });
  });
