...
describe('real Parent pilot sample lifecycle', () => {
  ...
  it('launches and clears the isolated sample with both pilot and demo-entry flags enabled', async () => {
    ...
    const { usePrototypeStore, serviceRegistry } = await loadStore('supabase', storage);
    usePrototypeStore.getState().setLocale('en');
    assertOk(await usePrototypeStore.getState().startPilotSample());
    expect(usePrototypeStore.getState()).toMatchObject({
      ...
      pilotSampleActive: true,
      locale: 'en',
      localFamily: { record: { schemaVersion: 4, children: [{ sex: 'male' }, { sex: 'female' }] } },
    });
    assertOk(
    ...
    assertOk(usePrototypeStore.getState().setAmbientSoundEnabled(true));
    assertOk(usePrototypeStore.getState().signOutExperience());
    const entry = {
      principal: 'child_salem' as const,
      expectedGeneration: usePrototypeStore.getState().demoRunGeneration,
      expectedEpoch: usePrototypeStore.getState().demoEntryEpoch,
    };
    assertOk(usePrototypeStore.getState().enterDemoExperience(entry));
    ...
    assertOk(await usePrototypeStore.getState().startPilotSample());
    assertOk(usePrototypeStore.getState().authorizeParentExperience());
    expect(storage.getItem).not.toHaveBeenCalled();
    expect(storage.setItem).not.toHaveBeenCalled();
    expect(storage.removeItem).not.toHaveBeenCalled();
    expect(device.getItem(LOCAL_FAMILY_STORAGE_KEY)).toBe('untouched device family');
