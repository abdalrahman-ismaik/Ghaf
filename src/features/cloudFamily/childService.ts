...
import { createProjectAccountStorage } from '@/services/accounts';
...
async function projectStorage() {
  ...
  validateParentAccountConfiguration({
    url: config.supabaseUrl,
    publishableKey: config.supabasePublishableKey,
  });
  const { createPlatformAccountStorage } = await import('@/services/accounts/platformStorage');
  return createProjectAccountStorage(createPlatformAccountStorage(), config.supabaseUrl).storage;
}

export async function readCloudDeviceMode(): Promise<'parent' | 'child'> {
  return (await (await projectStorage()).getItem(MODE_KEY)) === 'child' ? 'child' : 'parent';
}

export async function writeCloudDeviceMode(mode: 'parent' | 'child') {
  await (await projectStorage()).setItem(MODE_KEY, mode);
}
...
export function createCloudChildService() {
  ...
  const getRuntime = () => {
    runtime ??= (async () => {
      ...
      const scoped = createProjectAccountStorage(createPlatformAccountStorage(), provider.url);
      const storage = new GuardedAccountStorage(scoped.storage, CHILD_STORAGE_KEY);
