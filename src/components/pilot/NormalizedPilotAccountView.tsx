...
export interface NormalizedPilotAccountViewProps {
...
export function NormalizedPilotAccountView({
  controller,
  state,
  children,
  onChildAccess,
}: NormalizedPilotAccountViewProps) {
      ...
      {phase === 'ready' ? (
        ...
        >
          {(['workspace', 'account', 'sample'] as const).map((panel) => (
            ...
            >
              {t(`normalizedCloudFamily.navigation.${panel}`)}
            </Button>
          ))}
        </View>
      ) : null}
        ...
        {phase === 'signin' ? (
          <>
            {onChildAccess ? (
              ...
              >
                {t('normalizedCloudFamily.access.childEntry')}
              </Button>
            ) : null}
            <Button
              brand
