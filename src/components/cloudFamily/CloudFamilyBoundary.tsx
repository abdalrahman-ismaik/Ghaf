    ...
    <View style={styles.root} testID="cloud-family-boundary">
      {state.error ? (
        <StatusBanner
          ...
          language={locale}
          tone="error"
          message={t(`normalizedCloudFamily.errors.${state.error}`)}
        />
      ) : null}
      {state.saved ? (
        <StatusBanner
          ...
          language={locale}
          tone="success"
          message={t('normalizedCloudFamily.saved')}
        />
      ) : null}
      {state.busy ? (
        <ActivityIndicator
          color={colors.ghafEmerald}
          accessibilityLabel={t('normalizedCloudFamily.loading')}
        />
      ) : null}
      {state.uncertain ? (
        <>
          <Text brand>{t('normalizedCloudFamily.uncertain')}</Text>
          <Button
            brand
            busy={state.busy}
      ...
      >
        {t('normalizedCloudFamily.refresh')}
      </Button>
      {state.error === 'reauthentication_required' && reauthenticate ? (
        <View style={styles.root}>
          <AccessTextField
            label={t('normalizedCloudFamily.access.password')}
            language={locale}
            direction={direction}
            value={password}
          ...
          {authError ? (
            <StatusBanner
              ...
              language={locale}
              tone="error"
              message={t(`normalizedCloudFamily.errors.${authError}`)}
            />
          ) : null}
          <Button
            brand
          ...
          >
            {t('normalizedCloudFamily.access.confirm')}
          </Button>
        </View>
      ) : null}
      {authDone ? (
        <Text brand accessibilityLiveRegion="polite">
          {t('normalizedCloudFamily.access.reauthenticated')}
        </Text>
      ) : null}
      {state.snapshot ? (
        <View
        ...
        >
          {state.snapshot.legacy_available && expected.role === 'parent' ? (
            <View style={styles.root}>
              <Text brand>{t('normalizedCloudFamily.imported')}</Text>
              <Button
                brand
              ...
              >
                {t('normalizedCloudFamily.import')}
              </Button>
            </View>
          ) : null}
          <CloudFamilyScreen
