...
- Remaining regression suite with the two unrelated portrait provenance files excluded: `PASSED` —
  126 files, 1,382 tests.
- Full `npm test`: `FAILED` — 1,389 of 1,391 tests passed. The two failures are the existing
  `tests/access-family-portraits.test.tsx` and `tests/parent-access-portrait.test.tsx` exact-string
  checks against concurrently modified provenance files outside this feature's ownership.
