...
## 2026-09-02 Family Experience Frontend and Android Window

**Integration owner**: `/root`
**Read-only auditors**: `/root/markdown_gap_audit`, `/root/android_readiness_audit`, and
`/root/frontend_quality_audit`
**Scope**: reconcile the current Markdown with the implementation, expose the completed synthetic
access, private Family Reward, and synthetic Family League domains inside the existing ten routes,
and improve source-verifiable Android/mobile behavior without adding production capabilities

| Owner / workstream                                                                      | Exact reserved boundary                                                                                                                                                                                                                                                                                                                                   | Handoff condition                                                                                                                                                                                                           |
| --------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/root` — specification and phased integration                                          | `TEAM_OWNERSHIP.md`, `PRODUCT.md`, `DESIGN.md`, `PROTOTYPE_LIMITATIONS.md`, `DEMO_RUNBOOK.md`, `specs/003-family-growth-garden/{spec.md,plan.md,tasks.md,redesign-gap-analysis.md,contracts/acceptance-contract.md,checklists/story-evidence.md,checklists/web-proxy.md}`, `src/i18n/resources.ts`, and `app/{role.tsx,parent/index.tsx,child/index.tsx}` | The new phase is authorized before behavior changes; the ten-route journey stays complete; shared state and copy integrate only service-authorized projections                                                              |
| `/root/family_store_integration` — family presentation state                            | `src/state/usePrototypeStore.ts` and `tests/family-experience-store.test.ts`                                                                                                                                                                                                                                                                              | Store actions expose controller projections, receipt synchronization, role entry, and exact reset without leaking credentials or mutating P0 counters                                                                       |
| `/root/family_experience_controller` — family experience application boundary           | `src/features/family/familyExperienceController.ts` and `tests/family-experience-presentation.test.ts`                                                                                                                                                                                                                                                    | Synthetic access can be revoked/restored, private rewards derive only from authoritative recognition receipts, and League state remains separate from Seeds, Garden, and Green Circle                                       |
| `/root` — reusable presentation                                                         | `src/components/family-growth/{SyntheticAccessPanel.tsx,FamilyRewardPanel.tsx,FamilyLeaguePanel.tsx}` and `tests/family-experience-ui.test.ts`                                                                                                                                                                                                            | Parent and Child views use the existing tokens, bilingual resources, direct labels, logical layout, accessible controls, and truthful synthetic/private disclosures                                                         |
| `/root/android_runtime_hardening` — Android package/config verification                 | `app.config.ts`, `package.json`, `package-lock.json`, and `tests/android-runtime-readiness.test.ts`                                                                                                                                                                                                                                                       | Expo packages are aligned to SDK 57; unused real-audio/form packages are removed; permissions, backup, keyboard, predictive Back, and route inventory have static evidence                                                  |
| `/root/mobile_frontend_hardening` — direction, navigation, hierarchy, and accessibility | `src/i18n/index.ts`, `app/_layout.tsx`, `src/components/{LanguageSwitcher.tsx,journey.tsx,primitives.tsx}`, `src/components/family-growth/{SyntheticVoicePanel.tsx,ParentVoicePermissionPanel.tsx,ParentCheckIn.tsx,ParentPatternSummary.tsx}`, and `app/{child/task.tsx,parent/task/new.tsx,parent/task/review.tsx}`                                     | Runtime language changes have one locale-owned inherited root direction; Back/handoffs do not expose stale screens; optional actions, headings, narrow headers, voice states, and token use meet the approved source checks |
| `/root/android_asset_optimizer` — prepared image payload                                | `assets/images/fixture-recycling-clean-v1.{png,webp,md}`, `src/components/demoAssets.ts`, and `tests/android-asset-readiness.test.ts`                                                                                                                                                                                                                     | A visually checked replacement materially reduces the packaged image payload without changing dimensions, fixture meaning, accessibility text, or synthetic provenance                                                      |

All auditors are read-only and hold no write boundary. Any later worker receives one disjoint row
before editing, shares this worktree, preserves other changes, does not commit, and returns the
...
**Family experience frontend release — 2026-09-02**: every reservation above is released to
`/root`; no reviewer or writer retains a file boundary. Controller, store, presentation, Android
configuration, image, and review-correction slices are committed, including final corrections
`348620d`, `a05e6e4`, and `e4d77ad`. The aggregate verification passes typecheck, lint, formatting,
30 files / 464 tests, Expo dependency alignment, and 12 static pages. The final Android export is
